(() => {
  ...
  var StackGallery = class _StackGallery {
    ...
    loadItems(container) {
      this.items = [];
      const figures = container.querySelectorAll("figure.gallery-image, figure.gallery-video");
      for (let i = 0; i < figures.length; i++) {
        ...
        if (video) {
          ...
          const width = parseInt(video.getAttribute("width") || "640");
          const height = parseInt(video.getAttribute("height") || "360");
          let html = `<div class="pswp-video-wrap"><video controls playsinline style="max-width:100%;height:auto;" src="${src}"`;
          ...
          if (figcaption) aux.title = figcaption.innerHTML;
          this.items.push(aux);
        } else if (img) {
          let aux = {
    ...
    static createGallery(container) {
      ...
      let currentGallery = [];
      for (let i = 0; i < figuresEl.length; i++) {
        const figure = figuresEl[i];
    ...
    open(index) {
      ...
      const items = this.items.map((i) => {
        ...
        return { src: i.src, w: i.w, h: i.h };
      });
      const ps = new window.PhotoSwipe(pswp, window.PhotoSwipeUI_Default, items, {
        index,
    ...
    bindClick() {
      for (let index = 0; index < this.items.length; index++) {
        ...
        const localAnchors = Array.from(item.el.querySelectorAll("a"));
        const globalAnchors = Array.from(document.querySelectorAll(`a[href="${item.src}"]`));
        const anchors = localAnchors.concat(globalAnchors);
  ...
  // ns-hugo-imp:D:\daimacang\website_blog\hugo-test\themes\hugo-theme-stack-3.30.0\assets\ts\createElement.ts
